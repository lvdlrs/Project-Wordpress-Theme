...
// Setup (variable or functions available throughtout the theme)
define( 'JU_DEV_MODE', true );

// Includes (including any files)
...
add_action( 'show_user_profile', 'ju_custom_user_profile_fields' );//this will show users their profile in the admin dashboard when viewed
add_action( 'edit_user_profile', 'ju_custom_user_profile_fields' );//this will show profile on the admin profile
add_action( 'personal_options_update', 'ju_save_extra_profile_fields' );//this will add new fields in the users profile
add_action( 'edit_user_profile_update', 'ju_save_extra_profile_fields' );//this will add new fields in the admin profile
add_action( 'avatar_defaults', 'ju_new_avatar' );//adding defaults users avatar
add_action( 'pre_get_posts', 'ju_modify_home_page_query' );// this will modify the main query of a page
